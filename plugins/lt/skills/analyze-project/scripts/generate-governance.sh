#!/usr/bin/env bash
# Gera governanca contextual para um projeto alvo.
...
# Nao ha gerador de .codex/config.toml aqui: a projecao por host tem UM dono,
# plugins/lt/scripts/reconcile-hosts.py. A versao anterior carregava codex-config.sh sem
# chama-lo, e as duas copias do TOML ja divergiam (ordem de chaves raiz x tabelas).
...
DETECT_ARCH_SCRIPT="$ROOT_DIR/skills/agent-governance/scripts/detect-architecture.sh"
...
# Estrategia:
#   - Conteudo gerado e SEMPRE delimitado por marcadores lt:generated-{start,end}.
...
merge_write() {
  ...
  echo "AVISO: $file foi sobrescrito (backup: $file.bak) — sem marcadores harness lt na versao anterior." >&2
  return 0
}
...
has_manifest() {
  [[ -n "$(find_manifests "$1" 4)" ]]
}

has_any_files() {
  local dir="$1"
  [[ -d "$PROJECT_DIR/$dir" ]] || return 1
  [[ -n "$(find "$PROJECT_DIR/$dir" -mindepth 1 -maxdepth 1 -print -quit)" ]]
}
...
detect_frameworks() {
  ...
  local joined
  joined="$(printf '%s\n' ${frameworks[@]+"${frameworks[@]}"} | awk '!seen[$0]++ { lines[++n]=$0 } END { for(i=1;i<=n;i++) { if(i>1) printf ", "; printf "%s", lines[i] } }')"
  printf '%s' "$joined"
}
...
detect_primary_stack() {
  ...
  printf '%s\n' ${parts[@]+"${parts[@]}"} | awk 'NR==1{printf "%s",$0; next} {printf ", %s",$0}'
}
...
build_language_rules() {
  printf '%s\n' 'Skills de linguagem nao fazem parte desta distribuicao. Use o toolchain real do projeto e nao invente uma skill ausente.'
}
...
build_validation_commands() {
  local lines=()
  local toolchain_script="$ROOT_DIR/skills/agent-governance/scripts/detect-toolchain.sh"
  local toolchain_json=""

  lines+=("Seguir Etapa 4 de \`agent-governance/SKILL.md\` como base canonica.")
  lines+=("")
...
PY
    ...
    if [[ ${#detected_lines[@]} -gt 0 ]]; then
      lines+=(${detected_lines[@]+"${detected_lines[@]}"})
      printf '%s\n' ${lines[@]+"${lines[@]}"}
      return
    fi
  fi
...
build_stack_section() {
  ...
  if should_include_go && file_exists "go.mod"; then
    lines+=("- Projeto com contexto Go detectado; use o toolchain Go configurado no repositorio.")
  ...
  if should_include_node && (file_exists "package.json" || file_exists "tsconfig.json"); then
    lines+=("- Projeto com contexto Node/TypeScript detectado; use o toolchain Node configurado no repositorio.")
  ...
  if should_include_python && (file_exists "pyproject.toml" || file_exists "requirements.txt" || file_exists "setup.py" || file_exists "Pipfile"); then
    lines+=("- Projeto com contexto Python detectado; use o toolchain Python configurado no repositorio.")
    lines+=("- Validar versao de Python em \`pyproject.toml\` ou \`.python-version\` antes de usar APIs recentes.")
    has_stack=1
  fi

  if should_include_dotnet; then
    lines+=("- Projeto com contexto .NET/C# detectado; use o toolchain .NET configurado no repositorio.")
...
render_template() {
  ...
  local content
  content="$(env ${env_args[@]+"${env_args[@]}"} RENDER_COUNT="$pair_count" awk '
    BEGIN {
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  ...
  [[ "$INSTALL_CODEX" == "1" ]]   && echo "[dry-run] Geraria .codex/config.toml e .agents/skills/"
  [[ "$INSTALL_COPILOT" == "1" ]] && echo "[dry-run] Geraria .github/copilot-instructions.md"
  [[ "$INSTALL_OPENCODE" == "1" ]] && echo "[dry-run] Geraria .opencode/plugin/lt-governance.js"
else
  ...
  AGENTS_OPEN="<!-- lt:generated-start v${GOVERNANCE_SCHEMA_VERSION} -->"
  AGENTS_CLOSE="<!-- lt:generated-end -->"
  ...
  TOOL_OPEN_MD="<!-- lt:generated-start v${GOVERNANCE_SCHEMA_VERSION} -->"
  TOOL_CLOSE_MD="<!-- lt:generated-end -->"

  if [[ "$INSTALL_CLAUDE" == "1" ]]; then
    _claude_content="$(render_template "$AI_TOOL_TEMPLATE" \
      "TOOL_NAME" "Claude Code" \
      "TOOL_INSTRUCTION" "fonte canonica das regras" \
      "CONFIG_LINE_2" "As skills sao entregues pelo plugin \`lt@lt\`." \
      "CONFIG_LINE_3" "Os agents \`task-executor\` e \`reviewer\` sao entregues pelo mesmo plugin." \
      "SECAO_STACK" "$STACK_SECTION")"
    merge_write "$PROJECT_DIR/CLAUDE.md" "$TOOL_OPEN_MD" "$TOOL_CLOSE_MD" "$_claude_content"
  fi

  _hosts=""
  [[ "$INSTALL_CODEX" == "1" ]] && _hosts="codex"
  [[ "$INSTALL_COPILOT" == "1" ]] && _hosts="${_hosts:+$_hosts,}copilot"
  [[ "$INSTALL_OPENCODE" == "1" ]] && _hosts="${_hosts:+$_hosts,}opencode"
  if [[ -n "$_hosts" ]]; then
    # Fora do Claude a skill roda da copia materializada em .agents/skills/, onde o reconciler
    # nao existe. Falhar em voz alta: silenciar deixaria o host sem adaptador achando que tem.
    if [[ -f "$ROOT_DIR/scripts/reconcile-hosts.py" ]]; then
      python3 "$ROOT_DIR/scripts/reconcile-hosts.py" install --project "$PROJECT_DIR" --hosts "$_hosts"
    else
      echo "ERRO: reconcile-hosts.py ausente; rode scripts/install.sh --hosts $_hosts --project $PROJECT_DIR a partir do repo do harness" >&2
      exit 1
    fi
  fi
fi
