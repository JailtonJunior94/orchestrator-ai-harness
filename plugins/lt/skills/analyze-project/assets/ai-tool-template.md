...
Sempre leia o `references/INDEX.yaml` da skill carregada — ele mapeia cada reference para `file_patterns` e `diff_signals`. **Carregue references/*.md apenas quando o escopo da tarefa casar com a entrada do INDEX** (refs marcadas `always: true` sempre entram). Atalho: `bash ${CLAUDE_PLUGIN_ROOT}/scripts/resolve-references.sh <skill> <arquivos...>` lista exatamente o que carregar (le diff em stdin). Gate bloqueante quando a skill obrigatoria nao esta acessivel: `bash ${CLAUDE_PLUGIN_ROOT}/scripts/validate-skill-prerequisites.sh <arquivos...>`.
