...
2. Confirmar que `.lt/specs/prd-<feature-slug>/prd.md` e `.lt/specs/prd-<feature-slug>/techspec.md` existem.
...
3. Agrupar o trabalho em fatias que entreguem valor verificável.
   - **Fatiamento materialmente ambíguo** (granularidade/ordem/paralelismo com trade-offs reais): aplicar `skill lt:agent-governance, referencia multiple-choice-protocol.md` (2–5 opções, "(Recomendado)", uma pergunta por turno) antes de fixar o plano.
...
1. Após a aprovação, criar `.lt/specs/prd-<feature-slug>/tasks.md` a partir de `assets/tasks-template.md`.
...
7. Não escrever placeholders `PENDING-RUN` para hashes. Ao final da geração de `tasks.md`, sincronizar os comentários de rastreabilidade via CLI portátil usando o caminho do arquivo:
   - `bash "${CLAUDE_PLUGIN_ROOT}/scripts/lt-sdd.sh" sync-spec-hash .lt/specs/prd-<feature-slug>/tasks.md`
   ...
   Estes hashes permitem detectar drift posterior via `bash "${CLAUDE_PLUGIN_ROOT}/scripts/lt-sdd.sh" check-spec-drift .lt/specs/prd-<feature-slug>/tasks.md` ou `bash "${CLAUDE_PLUGIN_ROOT}/scripts/lt-sdd.sh" check-spec-drift .lt/specs/prd-<feature-slug>`.
...
1. Listar o diretório `${CLAUDE_PLUGIN_ROOT}/skills/` para enumerar todas as skills disponíveis no projeto. Classificar cada skill **lendo o campo `category` do frontmatter** de `skill lt:<skill>/SKILL.md` (fonte de verdade — não usar lista hardcoded em prosa):
...
2. Para cada skill restante, ler `description` no frontmatter de `skill lt:<skill>/SKILL.md`.
...
5. **Regras agnósticas obrigatórias:**
   - Nunca inventar skill que não exista em `${CLAUDE_PLUGIN_ROOT}/skills/`. Validar presença lendo o diretório antes de listar.
...
2. Marcar dependências críticas explicitamente. Formato canônico da coluna `Dependências`:
   ...
   - **Dependência cross-PRD** (opcional): use prefixo `<outro-slug>/` antes do id. Exemplo: `1.0, foundations/3.0, observability/2.0`. O orquestrador (`execute-all-tasks`) resolve o prefixo com `lt-sdd.sh specs-root --slug <outro-slug>` e exige que aquela tarefa esteja `done` antes de tornar a atual `ready`.
   - **Cross-PRD em OUTRO repositório**: declare o mapeamento em `.lt/config.yaml` do repo consumidor e o prefixo resolve lá:
     ...
     Existe porque uma feature que atravessa repos tem o PRD de quem consome dependendo do PRD de quem produz, e os dois não podem viver no mesmo lugar sem romper I-5. Sem o mapeamento, o prefixo resolve no repo atual — que é o comportamento certo para bundles irmãos do mesmo produto. O bundle mapeado **não** pode ser criado daqui (`--create` é recusado com exit 3): ele nasce rodando o comando dentro daquele repositório.
...
1. Rodar, após `sync-spec-hash`, os **dois** comandos — eles respondem perguntas diferentes:
   - `bash "${CLAUDE_PLUGIN_ROOT}/scripts/lt-sdd.sh" check-spec-drift .lt/specs/prd-<feature-slug>` — âncoras de hash entre PRD, techspec e tasks. **Não verifica cobertura de requisitos.**
   - `bash "${CLAUDE_PLUGIN_ROOT}/scripts/lt-sdd.sh" validate-sdd .lt/specs/prd-<feature-slug>` — schema da tabela, DAG e **cobertura de `RF-nn`/`REQ-nn`**.

   Os dois aceitam tanto o diretório do bundle quanto um arquivo dentro dele.
2. Se `validate-sdd` reportar requisito sem tarefa, voltar à tabela de `tasks.md` e aos arquivos `task-*.md` até que todos os `RF-nn`/`REQ-nn` de `prd.md` e `techspec.md` estejam cobertos.
3. Se o comando reportar hash ausente/divergente, rodar novamente `bash "${CLAUDE_PLUGIN_ROOT}/scripts/lt-sdd.sh" sync-spec-hash .lt/specs/prd-<feature-slug>/tasks.md` e repetir a validação.
4. Não reportar `done` enquanto `check-spec-drift` **e** `validate-sdd` não retornarem sem erro.
