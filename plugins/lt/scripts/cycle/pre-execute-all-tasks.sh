...
#
# Uso:
#   bash ${CLAUDE_PLUGIN_ROOT}/scripts/cycle/pre-execute-all-tasks.sh <prd-slug>
#
...
SDD="${CLAUDE_PLUGIN_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}/scripts/lt-sdd.sh"
if ! LT_PROJECT_DIR="$REPO_ROOT" bash "$SDD" assert-approved "$PRD_DIR" tasks; then
  echo "FAIL: tasks.md ou seus predecessores nao preservam a aprovacao registrada" >&2
  exit 1
fi
if ! LT_PROJECT_DIR="$REPO_ROOT" bash "$SDD" check-spec-drift "$PRD_DIR"; then
  echo "FAIL: drift entre PRD, TechSpec e tasks.md" >&2
  exit 1
fi
if ! LT_PROJECT_DIR="$REPO_ROOT" bash "$SDD" validate-sdd "$PRD_DIR"; then
  echo "FAIL: schema, DAG ou cobertura SDD invalida" >&2
  exit 1
fi
...
# O guard aqui era `command -v harness lt`, herdado do harness de origem, que instalava um
# binario. Este plugin deliberadamente NAO instala binario nenhum — o hash sai de
# `scripts/lt-sdd.sh`. O resultado e' que o guard reprovava em toda maquina limpa, o caminho que
# funciona nunca era tentado, e a funcao caia num fallback `go run` que so existe no repo do
# proprio harness: cross-PRD quebrava com "instale/atualize harness lt" tendo o script correto
# a um diretorio de distancia. Checar o arquivo que vai ser executado, nao um binario que nao
# existe.
hash_file() {
  local file="$1"
  local hash
  local dispatcher="${CLAUDE_PLUGIN_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}/scripts/lt-sdd.sh"
  if [[ -f "$dispatcher" ]]; then
    hash=$(bash "$dispatcher" hash "$file" 2>/dev/null || true)
    if [[ -n "$hash" ]]; then
      echo "$hash"
      return 0
    fi
  fi
  echo "FAIL: nao foi possivel calcular o hash de $file — '$dispatcher' ausente ou falhou. Rode 'lt:lt-doctor'." >&2
  return 1
}
...
for i in "${!IDS[@]}"; do
  ...
  for dep in "${DEP_LIST[@]}"; do
    ...
    if [[ "$dep" =~ ^([a-z][a-z0-9_-]*)/([0-9]+\.[0-9]+)$ ]]; then
      ...
      # Concatenar prendia toda dependencia cross-PRD ao repo atual. Uma feature que atravessa
      # repositorios — o consumidor depende do produtor — nao tem como ser expressa assim sem
      # romper I-5. O resolvedor consulta `spec_repos:` e devolve o diretorio certo, seja ele
      # local ou do repo vizinho declarado.
      ext_dir="$(LT_PROJECT_DIR="$REPO_ROOT" bash "${CLAUDE_PLUGIN_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}/scripts/lt-sdd.sh" specs-root --slug "$ext_slug" 2>/dev/null)"
      if [[ -z "$ext_dir" ]]; then
        echo "FAIL F18: nao foi possivel resolver o bundle de '$ext_slug' (referenciado por $tid)" >&2
        errors=$((errors+1))
        continue
      fi
        ...
        # Bundle externo que ainda nao existe e tarefa externa que ainda nao esta `done` sao
        # ATRASO, nao corrupcao: reprovar aqui travava o PRD inteiro por causa de uma tarefa, e
        # as independentes so' rodavam a mao. Viram aviso; `lt-sdd.sh waves` deixa a dependente
        # bloqueada com o motivo. Referencia errada, drift e ciclo continuam reprovando.
        if [[ ! -f "$ext_tasks_md" ]]; then
          echo "WARN F18: bundle cross-PRD ainda nao existe: $ext_tasks_md — $tid fica bloqueada ate ele existir com $ext_slug/$ext_task_id done" >&2
          warnings=$((warnings+1))
          continue
        fi
        ...
        if [[ -z "$ext_status" ]]; then
          ...
          echo "WARN F18: cross-PRD task not done: $ext_slug/$ext_task_id status=$ext_status — $tid fica bloqueada" >&2
          warnings=$((warnings+1))
        fi
...
detect_cycle() {
  ...
  [[ "$depth" -gt 3 ]] && return 0
  local current_dir
  current_dir="$(LT_PROJECT_DIR="$REPO_ROOT" bash "$SDD" specs-root --slug "$current" 2>/dev/null || true)"
  [[ -z "$current_dir" ]] && return 0
  local target_md="$current_dir/tasks.md"
